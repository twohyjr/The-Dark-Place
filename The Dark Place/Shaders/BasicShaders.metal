...
float4 applyPhongLighting(float4 color, Material material, Light light, float3 surfaceNormal, float3 toLightVector, float3 toCameraVector);
...
fragment half4 basic_fragment_shader(const RasterizerData rd [[ stage_in ]],
    ...
    float4 litColor = applyPhongLighting(color, material, light, rd.surfaceNormal, toLightVector, rd.toCameraVector);
...
float4 applyPhongLighting(float4 color, Material material, Light light, float3 surfaceNormal, float3 toLightVector, float3 toCameraVector){
    ...
    color = (float4(diffuse, 1.0) + float4(finalSpecular, 1.0) + float4(ambient,1)) * light.brightness;
